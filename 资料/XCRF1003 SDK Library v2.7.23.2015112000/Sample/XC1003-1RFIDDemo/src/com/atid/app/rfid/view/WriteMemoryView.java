...
public class WriteMemoryView extends Activity
		implements OnItemSelectedListener, OnClickListener, RfidReaderEventListener {

	private static final String TAG = WriteMemoryView.class.getSimpleName();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.view_write_memory);
		...
		adpOffset = new SpinnerAdapter(this, android.R.layout.simple_list_item_1);
	...
	private void startAction() {
		...
		BankType bank = BankType.valueOf(adpBank.getValue(spnBank.getSelectedItemPosition()));
		...
		if ((res = mReader.writeMemory6c(bank, offset, data, password)) != ResultCode.NoError) {
			ATLog.e(TAG, "ERROR. startAction() - Failed to write memory {%s, %d, [%s], [%s]} - [%s]", bank, offset,
					(data == null ? "" : data), (password == null ? "" : password), res);
			mAction = false;
			return;
		}
		mAction = true;
		...
		ATLog.i(TAG, "INFO. startAction() - {%s, %d, [%s], [%s]}", bank, offset, (data == null ? "" : data),
